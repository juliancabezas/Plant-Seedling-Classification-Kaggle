...
# Project 2
# ResNeXt-10 32x16d model hyperparameter finetuning
####################################
...
n_epoch_f = 40 # Number of epochs of the first stage of training
n_epoch_uf = 40 # Number of epochs of the second stage of training
learning_rate_decrease = 0.075 # Factor to multiply the learning rate from the stage 1 to the stage 2 of training
...
# Use the ResNeXt-101 32x16d pretrained with Imagenet and Instagram. WSL (weakly supervised learning) means Instagram pretrained
model = 'Resnext_32x16d_101_IN_WSL'

# Test Extra data transformations, set to True to include them
vertical_flip = False
color_jitter = False
...
# Compose the full training data augmentation methods
model_name = model + '_' + str(n_epoch_f) + 'e_' + str(batch_size) + 'b_' + 'adam_lr' + str(learning_rate).replace('0.','') + '_finetuning'
model_name
...
train_transformations = [transforms.RandomResizedCrop(224),transforms.RandomHorizontalFlip()]

# Include the vertical flip or color jitter transformation if they are set to True
if vertical_flip:
    train_transformations.append(transforms.RandomVerticalFlip())

if color_jitter:
    train_transformations.append(transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.3))

# Append the data transformations that are used in all cases
train_transformations.append(transforms.RandomCrop(224, 10))
...
df_grid_search.to_csv(root_dir + 'results/' + model_name + '_' + 'train_val_acc' + '.csv')
...
# Generate a single numpy array with the predicted and observed data
